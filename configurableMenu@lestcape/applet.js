...
ButtonChangerBox.prototype = {
   ...
   _init: function (parent, icon, iconSize, labels, selected, callBackOnSelectedChange) {
      ...
      this.label = new St.Label({ text: labels[selected] });
...
//Config//
  ...
  _openOutScreen: function() {
   ...
   _updateMenuSection: function() {
      ...
      this.menu.actor.connect('button-release-event', Lang.bind(this, this._onMenuButtonRelease2));
   ...
   _onMenuButtonRelease2: function(actor, event) {
      try {
   ...
   _doResize: function() {
      ...
      if((this.actorResize)&&(this.relativeSide == this.actorResize._delegate._boxPointer.relativeSide)) {
         ...
         if(global.stage_input_mode != Cinnamon.StageInputMode.FULLSCREEN)
             Main.notify("res " + global.stage_input_mode + " " + Cinnamon.StageInputMode.FULLSCREEN);
...
//search
   ...
   _appletGenerateApplicationMenu: function(generate) {
      ...
      if(generate) {
         ...
         this.appMenu.actor.connect('button-release-event', Lang.bind(this, this._onMenuButtonRelease2));
